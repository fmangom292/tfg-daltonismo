...
export class TestMullerLyerComponent implements OnInit {
  ...
  readonly configService = inject(TiempoReaccionService);
  ...
  stimulusNumber: number = 250; // Tiempo en milisegundos para mostrar el estímulo
  ...
  setLine1Length(length: number): void {
    this.line1Length = length;
  }

  setLine2Length(length: number): void {
  ...
  changeLine1Width(){
    ...
    this.line1Length = this.getRandomLineLength();
    this.line1Color = this.testService.getRandomColor().hex; // Cambia el color de la línea 1
    if(this.line1Length === this.line2Length) {
      this.line1Length = this.getRandomLineLength(); // Asegúrate de que las longitudes no sean iguales
    }

    console.log(this.line1Color);
